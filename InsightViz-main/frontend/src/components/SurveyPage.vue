<template>
  <div class="surveys">
          ...
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                <label for="typeEntite">Type d'entité *</label>
                <select id="typeEntite" v-model="surveyForm.typeEntite" class="form-select" required>
              ...
              <div class="form-group">
                ...
                <input 
                  id="nomEntreprise" 
                  v-model="surveyForm.nomEntreprise" 
                  type="text" 
                  required 
                  class="form-input"
            ...
            <tr v-for="survey in filteredSurveys" :key="survey.id" class="table-row">
              ...
              <td>
                <div class="entity-info">
                  <span class="entity-badge" :class="survey.typeEntite">
                    {{ survey.typeEntite === 'transporteur' ? 'Transporteur' : 'Expéditeur' }}
                  </span>
                  <p class="company-name">{{ survey.nomEntreprise }}</p>
                </div>
              </td>
              <td>
    ...
    <div class="mobile-view">
      <div v-for="survey in filteredSurveys" :key="survey.id" class="survey-card">
        ...
        <div class="card-content">
          <div class="card-row">
            <span class="card-label">Type & Entreprise:</span>
            <div class="entity-info">
              <span class="entity-badge" :class="survey.typeEntite">
                {{ survey.typeEntite === 'transporteur' ? 'Transporteur' : 'Expéditeur' }}
              </span>
          ...
          <div class="card-row">
            <span class="card-label">Campagne:</span>
            <div>
              <span v-if="survey.campagne" class="campaign-badge">
                {{ getCampaignName(survey.campagne) }}
              </span>
          ...
          <div class="card-row">
            ...
            <span class="card-value">{{ survey.questions.length }}</span>
          </div>
          
          <div class="card-row">
            <span class="card-label">Réponses:</span>
            <span class="card-value">{{ survey.reponses }}</span>
          </div>
          ...
          <div class="card-row">
            ...
            <span class="card-value">{{ survey.dateCreation }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="filteredSurveys.length === 0 && searchQuery" class="no-results">
      <p>Aucune enquête trouvée pour "{{ searchQuery }}"</p>
    </div>
  </div>
</template>
...
interface Survey {
  ...
  description: string
  campagne: string
  typeEntite: 'transporteur' | 'expediteur'
...
const surveyForm = ref({
  ...
  description: '',
  campagne: '',
  typeEntite: 'transporteur' as 'transporteur' | 'expediteur',
  nomEntreprise: '',
...
const surveys = ref<Survey[]>([
  {
    ...
    campagne: 'campagne-1',
    typeEntite: 'transporteur',
    nomEntreprise: 'Transport Express',
    questions: [
  ...
  },
  {
    ...
    campagne: 'campagne-3',
    typeEntite: 'expediteur',
    nomEntreprise: 'Logistique Plus',
    questions: [
  ...
  },
  {
    ...
    campagne: 'campagne-2',
    typeEntite: 'transporteur',
    nomEntreprise: 'Fret International',
    questions: [
...
const filteredSurveys = computed(() => {
  return surveys.value.filter(survey => 
    ...
    survey.nomEntreprise.toLowerCase().includes(searchQuery.value.toLowerCase())
  )
})
...
const editSurvey = (survey: Survey) => {
  ...
  surveyForm.value = {
    ...
    typeEntite: survey.typeEntite,
    nomEntreprise: survey.nomEntreprise,
...
.entity-badge.transporteur {
  background-color: #dbeafe;
  color: #3b82f6;
}

.entity-badge.expediteur {
